...
module.exports = app => {
  ...
  app.post('/add_new_tech', (req, res) => {
    ...
    connection.query(sql, req.body.tech_id, (error, rows) => {
      ...
      if (rows.length > 0) {
        res.status(200).send(JSON.stringify("User already exist"))
      }
      else {
        ...
        connection.query(sql, tech_body, (err, results) => {
          ...
          res.status(200).send(JSON.stringify(  "Successfully added to the system" )
          
           )
          }
          else{
            res.status(400).send(JSON.stringify( "Unable to add to the system")
